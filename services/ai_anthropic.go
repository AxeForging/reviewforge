...
func (p *AnthropicProvider) Review(systemPrompt, userPrompt string, temperature float64) (string, error) {
	baseURL := p.BaseURL
	if baseURL == "" {
	...
	if resp.StatusCode != http.StatusOK {
		return "", fmt.Errorf("%w: Anthropic returned %d: %s", helpers.ErrAIRequest, resp.StatusCode, string(respBody))
	}
	...
	if len(result.Content) == 0 {
		return "", fmt.Errorf("%w: Anthropic returned no content", helpers.ErrAIRequest)
	}
